...
function fetchMetroLines() {
    ...
    const mockLine = {
      ...
      stations: [
        ...
        { stationName: "Ba Son" },
        { stationName: "Suoi Tien Terminal" }
      ]
    };
    ...
    container.innerHTML = `
      <div class="card mb-4">
        <div class="card-header bg-primary text-white">
          <h3 class="card-title mb-0"><i class="fas fa-subway me-2"></i>${mockLine.lineName}</h3>
        </div>
        <div class="card-body">
          <div class="d-flex justify-content-between mb-3">
            <div><strong>Status:</strong> <span class="badge bg-success">Operational</span></div>
            <div><strong>Duration:</strong> ${mockLine.totalDuration} minutes</div>
            <div><strong>First/Last Train:</strong> ${formatTime(mockLine.firstDeparture)} / 10:00 PM</div>
          </div>
          <p class="mb-3">Connects Ben Thanh to Suoi Tien Terminal, passing through key city locations.</p>
          <div class="metro-map mb-4">
            <h5>Route Map</h5>
            <div class="station-route">
              <div class="station-list">
                ${mockLine.stations.map((s, i) => `
                  <div class="station${i === 0 ? ' start-station' : i === mockLine.stations.length - 1 ? ' end-station' : ''}">
                    <div class="station-marker"></div>
                    <div class="station-info">
                      <div class="station-name">${s.stationName}</div>
                      <div class="station-time">${i === 0 ? 'Departure' : i === mockLine.stations.length - 1 ? 'Arrival' : ''}</div>
                    </div>
                  </div>
                `).join('')}
              </div>
          ...
          <div class="upcoming-trips mt-4">
            ...
            <div class="trip-cards">
              <div class="trip-card">
                <div class="departure-time">06:30 AM</div>
                <div class="trip-info">
                  <div>Ben Thanh → Suoi Tien Terminal</div>
                  <div class="small text-muted">Arrives: 07:00 AM</div>
                </div>
...
async function fetchAndRenderTicketTypes() {
  ...
  console.log("Ticket loaded:", ticket.code);

  const isOneWay = ticket.code.startsWith("ONE_WAY");
  ...
  card.innerHTML = `
    ...
    <div class="d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-outline-success btn-sm purchase-btn" ${isOneWay ? '' : 'disabled'}>
          Purchase
        </button>
      </div>
    </div>
  ...
  //  Purchase logic for any ticket starting with 'ONE_WAY'
  const purchaseBtn = card.querySelector('.purchase-btn');
  if (isOneWay) {
    purchaseBtn.addEventListener('click', () => {
      if (!isLoggedIn()) {
        if (confirm("You need to sign in to purchase. Redirect now?")) {
          window.location.href = "signin.html";
        }
        return;
      }

      const fromStation = "ben-thanh";
      const toStation = "suoi-tien";

      document.getElementById('modalFromStation').innerHTML = `<option value="${fromStation}">${fromStation}</option>`;
      document.getElementById('modalToStation').innerHTML = `<option value="${toStation}">${toStation}</option>`;
...
document.getElementById('oneWayTicketForm').addEventListener('submit', async function (e) {
    e.preventDefault();
  
    const ticketCode = "ONE_WAY_4"; // 🟢 Or dynamically store this based on selected ticket if needed

const res = await fetch(`http://localhost:8080/api/tickets/purchase?ticketTypeCode=${ticketCode}`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${localStorage.getItem('jwtToken')}` // 🟢 Ensure token is attached
  },
  body: JSON.stringify({
